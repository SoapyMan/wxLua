...
wxLuaBindMethod* wxLuaBinding::GetClassMethod(const wxLuaBindClass *wxlClass, const char *methodName, int method_type, bool search_baseclasses )
{
    ...
    {
		// found methods list
		std::vector<wxlFoundMethod> foundMethods;
        ...
        for (size_t i = 0; wxlClass->baseclassNames[i]; ++i)
        {
            ...
            {
				wxlFoundMethod fmethod = GetClassMethod_r(wxlClass->baseBindClasses[i], methodName, method_type, 0);
                if (fmethod.method != NULL)
					foundMethods.push_back( fmethod );
            }
        }

		short minDepth = 16384;
		wxLuaBindMethod* bestMethod = NULL;

		// not a serious fix
		for(size_t i = 0; i < foundMethods.size(); i++)
		{
			if(foundMethods[i].depth < minDepth)
			{
				bestMethod = foundMethods[i].method;
				minDepth = foundMethods[i].depth;
			}
		}
...
//static
wxlFoundMethod wxLuaBinding::GetClassMethod_r(const wxLuaBindClass *wxlClass, const char *methodName, int method_type, int depth)
{
	wxlFoundMethod retMethod(NULL,16384);

    wxCHECK_MSG(wxlClass, retMethod, wxT("Invalid wxLuaBindClass to find method from."));
    ...
    if ((wxlMethod == NULL) && wxlClass->baseclassNames)
    {
        for (size_t i = 0; wxlClass->baseclassNames[i]; ++i)
        {
            // The class may not have been installed
            if (wxlClass->baseBindClasses[i])
            {
				wxlFoundMethod fmethod = GetClassMethod_r(wxlClass->baseBindClasses[i], methodName, method_type, depth+1);
                if (fmethod.method != NULL)
					return fmethod;
            }
        }
	...
	if(wxlMethod)
	{
		retMethod.method = wxlMethod;
		retMethod.depth = depth;
	}

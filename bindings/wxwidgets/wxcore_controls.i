...
class wxTextCtrl : public wxControl
{
	...
	void SetHint(const wxString& value );
...
#if wxLUA_USE_wxTreeListCtrl && wxUSE_TREELISTCTRL
...
class wxTreeListCtrl : public wxControl
{
    ...
    wxTreeListItem AppendItem(wxTreeListItem parent, const wxString& text, int imageClosed = -1, int imageOpened = -1, %ungc wxLuaTreeItemData* data = NULL);		  
	wxTreeListItem InsertItem(wxTreeListItem parent, wxTreeListItem previous, const wxString& text, int imageClosed = -1, int imageOpened = -1, %ungc wxLuaTreeItemData* data = NULL);
    wxTreeListItem PrependItem(wxTreeListItem parent, const wxString& text, int imageClosed = -1, int imageOpened = -1, %ungc wxLuaTreeItemData* data = NULL);
	...
	void SetItemBackgroundColour(const wxTreeListItem& item, const wxColour& col );
	wxColour GetItemBackgroundColour(const wxTreeListItem& item);
	
	void SetItemTextColour(const wxTreeListItem& item, const wxColour& col );
	wxColour GetItemTextColour(const wxTreeListItem& item);

    wxLuaTreeItemData* GetItemData(wxTreeListItem item) const;
    void SetItemData(wxTreeListItem item, %ungc wxLuaTreeItemData* data);
...
// FAKE typedef, actually typedef void* wxTreeListItemValue
// Since we override the functions that use it we handle it as a pointer.
typedef double wxTreeListModelNode

// ---------------------------------------------------------------------------
// wxTreeListItem - wxTreeListCtrl
...
class %delete wxTreeListItem
{
    ...
    bool IsOk( );
    wxTreeListModelNode GetID() const; // get a pointer to the internal data to use as a reference in a Lua table
};

class %delete wxTreeListEvent : public wxNotifyEvent
{
	%wxEventType  wxEVT_COMMAND_TREELIST_SELECTION_CHANGED   // EVT_TREELIST_SELECTION_CHANGED(id, fn );
	%wxEventType  wxEVT_COMMAND_TREELIST_ITEM_EXPANDING      // EVT_TREELIST_ITEM_EXPANDING(id, fn );
	%wxEventType  wxEVT_COMMAND_TREELIST_ITEM_EXPANDED       // EVT_TREELIST_ITEM_EXPANDED(id, fn );
	%wxEventType  wxEVT_COMMAND_TREELIST_ITEM_CHECKED        // EVT_TREELIST_ITEM_CHECKED(id, fn );
	%wxEventType  wxEVT_COMMAND_TREELIST_ITEM_ACTIVATED      // EVT_TREELIST_ITEM_ACTIVATED(id, fn );
	%wxEventType  wxEVT_COMMAND_TREELIST_ITEM_CONTEXT_MENU   // EVT_TREELIST_ITEM_CONTEXT_MENU(id, fn );
	%wxEventType  wxEVT_COMMAND_TREELIST_COLUMN_SORTED       // EVT_TREELIST_COLUMN_SORTED(id, fn );
